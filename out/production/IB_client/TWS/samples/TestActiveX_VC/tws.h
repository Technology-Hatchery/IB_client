...
public:
	...
	void placeOrder2(long id, LPCTSTR action, long quantity, LPCTSTR localSymbol,
		...
		LPCTSTR faMethod, LPCTSTR faPercentage, LPCTSTR faProfile, LPCTSTR goodTillDate);
	void disconnect();
	void connect(LPCTSTR host, long port, long clientId);
	...
	void reqMktDepthEx(LONG tickerId, IDispatch* contract, LONG numRows);
	...
	void reqRealTimeBarsEx(LONG tickerId, IDispatch* contract, LONG barSize, LPCTSTR whatToShow, BOOL useRTH);
	void reqScannerSubscriptionEx(LONG tickerId, IDispatch* subscription);
