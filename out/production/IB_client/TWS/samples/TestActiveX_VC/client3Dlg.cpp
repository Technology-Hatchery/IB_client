...
void CClient3Dlg::OnConnect() 
{
	...
	m_tws.connect( dlg.m_ipAddress, dlg.m_port, dlg.m_clientId);
...
void CClient3Dlg::OnReqMktData() 
{
    ...
    m_dlgOrder->init( "Request Market Data",
		CDlgOrder::REQ_MKT_DATA, m_managedAccounts);
...
void CClient3Dlg::OnCanMktData() 
{
    m_dlgOrder->init( "Cancel Market Data",
		CDlgOrder::CANCEL_MKT_DATA, m_managedAccounts);
...
void CClient3Dlg::placeOrder(bool whatIf)
{
    ...
    m_dlgOrder->init( "Place Order",
		CDlgOrder::ORDER, m_managedAccounts);
...
void CClient3Dlg::OnCanorder() 
{
    ...
    m_dlgOrder->init( "Cancel Order",
		CDlgOrder::CANCEL_ORDER, m_managedAccounts);
...
{
	CDlgScanner dlgScanner(m_scannerSubscr, &m_tws);
	dlgScanner.DoModal();
}
...
void CClient3Dlg::OnExerciseOptions()
{
    m_dlgOrder->init( "Exercise Options",
		CDlgOrder::EXERCISE_OPTIONS, m_managedAccounts);
...
void CClient3Dlg::OnReqHistoricalData() 
{
	...
	if ( whatToShow == "estimates" || whatToShow == "finstat" || whatToShow == "snapshot") {
		m_tws.reqFundamentalData( m_dlgOrder->m_id, m_dlgOrder->getComContract(), whatToShow);
		return;
	}
...
void CClient3Dlg::OnReqRealTimeBars()
{
    m_dlgOrder->init( "Request Real Time Bars",
		CDlgOrder::REQ_REAL_TIME_BARS, m_managedAccounts);
...
{
	// run dlg
	m_dlgOrder->init( "Calculate Implied Volatility", CDlgOrder::CALC_IMPL_VOL, m_managedAccounts);
	if( m_dlgOrder->DoModal() != IDOK) return;
...
{
	// run dlg
	m_dlgOrder->init( "Calculate Option Price", CDlgOrder::CALC_OPTION_PRICE, m_managedAccounts);
	if( m_dlgOrder->DoModal() != IDOK) return;
...
{
	// run dlg
	m_dlgOrder->init( "Cancel Calculate Implied Volatility", CDlgOrder::CANCEL_CALC_IMPL_VOL, m_managedAccounts);
	if( m_dlgOrder->DoModal() != IDOK) return;
...
{
	// run dlg
	m_dlgOrder->init( "Cancel Calculate Option Price", CDlgOrder::CANCEL_CALC_OPTION_PRICE, m_managedAccounts);
	if( m_dlgOrder->DoModal() != IDOK) return;
...
{
	// run dlg box
	m_dlgOrder->init( "Request Market Data Type", CDlgOrder::REQ_MARKET_DATA_TYPE, m_managedAccounts);
	if( m_dlgOrder->DoModal() != IDOK) return;
...
void CClient3Dlg::OnReqMktDepth() 
{
    ...
    m_dlgOrder->init( "Request Market Depth",
		CDlgOrder::REQ_MKT_DEPTH, m_managedAccounts);
...
void CClient3Dlg::OnCancelMktDepth() 
{
    ...
    m_dlgOrder->init( "Cancel Market Depth Data",
		CDlgOrder::CANCEL_MKT_DEPTH, m_managedAccounts);
