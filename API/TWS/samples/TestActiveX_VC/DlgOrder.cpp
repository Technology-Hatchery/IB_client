...
CDlgOrder::CDlgOrder(CWnd* pParent)
...
void CDlgOrder::setCtrl(CClient3Dlg* pClientDlg, CTws* pTws)
{
	m_pDlg = pClientDlg;
	m_pTws = pTws;

	{
		...
		dispContract.Attach(m_pTws->createContract());
	
		m_comContract = dispContract;
		if (m_comContract) {
			updateComContract();
		}
	}
	...
	{
		CComPtr<IDispatch> dispOrder;
		dispOrder.Attach(m_pTws->createOrder());
	
		m_comOrder = dispOrder;
		if (m_comOrder) {
			updateComOrder();
		}
	}
	...
	{
		CComPtr<IDispatch> dispUnderComp;
		dispUnderComp.Attach(m_pTws->createUnderComp());
	
		m_comUnderComp = dispUnderComp;
	}
}

void CDlgOrder::init(CString dlgTitle, DialogType dlgType, CString managedAccounts) 
{
    m_dlgTitle = dlgTitle;
    m_dlgType = dlgType;
...
void CDlgOrder::OnBtnAddComboLegs() 
{
	...
	if ( dlgComboLegs.DoModal() == IDOK ) {
		...
		PropsComboLeg::Put(m_comContract, comboLegs, &IContract::put_comboLegs);
		PropsOrderComboLeg::Put(m_comOrder, orderComboLegs, &IOrder::put_orderComboLegs);
	}
}
...
{
	CDlgUnderComp dlg(m_comUnderComp);
	const int nRet = dlg.DoModal();

	typedef CComPropAccessor<IContract> Props;

	if (nRet == IDOK) {
		Props::Put(m_comContract, m_comUnderComp, &IContract::put_underComp);
	}
	else if (nRet == IDRESET) {
		Props::Put(m_comContract, (IUnderComp*)NULL, &IContract::put_underComp);
	}
}
...
void CDlgOrder::OnBtnAlgoParams() 
{
	...
	Props::Get(algoStrategy, m_comOrder, &IOrder::get_algoStrategy);
	Props::GetQI(algoParams, m_comOrder, &IOrder::get_algoParams);

	CDlgAlgoParams dlgAlgoParams(algoStrategy, algoParams, m_pTws);
	if ( dlgAlgoParams.DoModal() == IDOK ) {
		algoStrategy = dlgAlgoParams.getAlgoStrategy();
		algoParams = dlgAlgoParams.getAlgoParams();
		Props::Put(m_comOrder, algoStrategy, &IOrder::put_algoStrategy);
		Props::Put(m_comOrder, algoParams, &IOrder::put_algoParams);
	}
}
...
void CDlgOrder::OnBtnSmartComboRoutingParams() 
{
	...
	Props::GetQI(smartComboRoutingParams, m_comOrder, &IOrder::get_smartComboRoutingParams);

	CDlgSmartComboRoutingParams dlgSmartComboRoutingParams(smartComboRoutingParams, m_pTws);
	if ( dlgSmartComboRoutingParams.DoModal() == IDOK ) {
		smartComboRoutingParams = dlgSmartComboRoutingParams.getSmartComboRoutingParams();
		Props::Put(m_comOrder, smartComboRoutingParams, &IOrder::put_smartComboRoutingParams);
	}
}
...
void CDlgOrder::updateComContract()
{
	ASSERT(m_comContract);

	IContract* comContract = m_comContract;

	typedef CComPropAccessor<IContract> Props;

#define PUT_PROP(Prop) \
	Props::Put(comContract, m_##Prop, &IContract::put_##Prop);
...
void CDlgOrder::updateComOrder()
{
	ASSERT(m_comOrder);

	IOrder* comOrder = m_comOrder;

	typedef CComPropAccessor<IOrder> Props;

	Props::Put(comOrder, m_id, &IOrder::put_orderId);
//	Props::Put(comOrder, m_clientId, &IOrder::put_clientId);
//	Props::Put(comOrder, m_permId, &IOrder::put_permId);

#define PUT_PROP(Prop) \
	Props::Put(comOrder, m_##Prop, &IOrder::put_##Prop);
